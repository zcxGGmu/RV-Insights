...
> **📖 新手？从这里开始 →** [**RV-Insights 使用教程系列(点击此处)**](./tutorial/tutorial.md) — 从零开始配置环境、连接大模型，3-5 分钟即可上手。

下一代集成通用 Agent 的 AI 桌面应用，支持对话、Agent、Agent Teams **等能力**，本地优先、多供应商支持、完全开源。支持远程通过飞书机器人与 Agent 对话和交互，甚至把 RV-Insights Agent 拉进群组替你完成工作，跟同事实现 Agent 协作，让你用手机也可以处理很多必要的工作。
...
- **AI Agent 模式** — 基于 Claude Agent SDK 0.2.120 的自主通用 Agent，支持工作区隔离和权限管理
...
**[下载 RV-Insights](https://github.com/ErlichLiu/RV-Insights/releases)**
...
进入 **设置 > 渠道管理**，点击 **添加渠道**，选择供应商并输入 API Key。RV-Insights 会自动填充正确的 API 地址。点击 **测试连接** 验证，然后 **获取模型** 加载可用模型列表。

### rv-pipeline 模式

rv-pipeline 需要配置两个渠道：
- **Claude 渠道**（用于 Explorer / Planner / Developer / Tester）：添加 Anthropic 或兼容 Claude Agent SDK 的供应商渠道
- **OpenAI 渠道**（用于 Reviewer）：添加 OpenAI 或兼容 OpenAI Agents SDK 的供应商渠道

进入 **设置 > rv-pipeline** 分别选择上述渠道和模型。流水线运行时，每个节点输出后会自动暂停，等待你在界面上审核确认后再进入下一阶段。
...
- [MemOS](https://memos.openmem.net) - RV-Insights 的记忆功能实现

## 参与贡献

欢迎大家参与 RV-Insights 的开发！无论是修复 Bug、新增功能还是改进文档，我们都非常欢迎你的贡献。

**PR 赠金活动** — RV-Insights 目前设有 PR 赠金计划，对合并的 PR 自动给予慷慨的赠金，支持在 Claude Code 等产品中使用，帮助大家更好地进行 AI 辅助开发。提交 PR 时请在描述中留下你的邮箱信息即可。
