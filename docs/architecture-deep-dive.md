# Proma 深度解析：一个开源 AI Agent 桌面应用的架构之道

> 本文将从架构师的视角，通俗易懂地拆解 Proma 这款开源 AI Agent 桌面应用的完整技术实现。我们会像拆解一辆跑车一样，从外到内、从整体到局部，逐层分析它的设计思想、核心模块和技术选型。

---

## 一、Proma 是什么？

Proma 是一个**开源的 AI Agent 桌面应用**，核心理念是"本地优先、通用 Agent"。你可以把它想象成一个装在你电脑里的"AI 管家"——它不仅能像 ChatGPT 一样和你聊天，还能像一位真正的助手那样：
...
如果把 Proma 比作一艘巨轮，它的架构设计就像**分舱设计**——不同功能区域被严格隔离，既保证安全，又便于维护和扩展。

### 2.1 Monorepo 结构

Proma 使用 Bun 的 Workspace 功能管理 Monorepo，整体分为三层：

```
proma/
├── packages/          # 共享库（像船的通用零件仓库）
│   ├── shared/        # @proma/shared — 类型定义、常量、工具函数
│   ├── core/          # @proma/core — AI Provider 适配器、代码高亮
│   └── ui/            # @proma/ui — 共享 React 组件（CodeBlock、MermaidBlock）
│
├── apps/              # 应用层（像船的各个功能舱室）
│   └── electron/      # @proma/electron — 完整的 Electron 桌面应用
│
└── web/               # 独立 Web 应用（轻量版）
```
...
```
@proma/shared (v0.1.17)     ← 零运行时依赖，纯 TypeScript
      ↓
@proma/core (v0.2.9)        ← 依赖 shared + shiki
      ↓
@proma/ui (v0.1.3)          ← 依赖 core + beautiful-mermaid + shiki
      ↓
@proma/electron (v0.9.11)   ← 依赖所有上层包 + 全部运行时依赖
```
...
Electron 应用的本质是**一个 Node.js 主进程 + 一个或多个 Chromium 渲染进程**。Proma 把这个模型用得淋漓尽致：
...
IPC（Inter-Process Communication，进程间通信）是 Electron 应用的核心命脉。Proma 设计了一套**四层同步的 IPC 架构**，这是整个项目最值得学习的设计之一。
...
Proma 有一个关键架构决策：**全局 IPC 监听器在应用顶层挂载，永不销毁**。
...
Proma 支持 10+ 个 AI 供应商（Anthropic、OpenAI、Google、DeepSeek、Moonshot、智谱、MiniMax、豆包、通义千问等），但代码里只有 **3 个适配器**。这是怎么做到的？
...
## 六、Agent SDK 集成：Proma 的心脏

如果说 Provider 适配器是翻译官，那 **Agent SDK** 就是 Proma 的"大脑"——它让 AI 从"聊天机器人"升级为"能动手做事的 Agent"。
...
Proma 使用 **Jotai** 而非 Redux、Zustand 或 Context API 管理状态。这是一个非常明智的选择。
...
这是 Proma 状态管理中最精妙的设计：**所有 per-session 状态使用 `Map<string, T>` 而非单值 atom**。
...
Proma 坚持"本地优先"原则——所有数据都存在你电脑的 `~/.proma/` 目录下，不用任何数据库。
...
Proma 的一个杀手级功能是**通过飞书/钉钉/微信机器人远程使用 Agent**。这意味着：
...
飞书消息 ↔ Proma 内部消息格式的转换由 `feishu-message.ts` 处理：
...
Proma 的构建链并不复杂，但有几个**非常值得注意的特殊挑战**：
...
*本文基于 Proma 开源代码分析撰写，技术细节截至 2025 年。*
