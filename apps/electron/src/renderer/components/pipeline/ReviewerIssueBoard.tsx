...
export function ReviewerIssueBoard({
  ...
  return (
    <section className="rounded-2xl border border-rose-200 bg-rose-50/70 px-4 py-4 text-rose-950 shadow-sm dark:border-rose-500/30 dark:bg-rose-500/10 dark:text-rose-100">
      <div className="flex items-start justify-between gap-3">
        <div>
          <div className="text-xs font-medium text-rose-700 dark:text-rose-200">Reviewer 结果</div>
          <h2 className="mt-1 text-base font-semibold">
            {viewModel.approved ? '审查通过' : '审查未通过'}
          </h2>
          <p className="mt-1 text-sm text-rose-800 dark:text-rose-200">{viewModel.summary}</p>
        </div>
        <div className="flex flex-shrink-0 flex-col items-end gap-1 text-xs text-rose-700 dark:text-rose-200">
          <span className="rounded-full bg-background/80 px-3 py-1 font-medium">{viewModel.iterationLabel}</span>
          <span>{viewModel.openIssueCount} 个待修复</span>
        </div>
      </div>

      {viewModel.reviewDocument ? (
        <div className="mt-3 rounded-xl bg-background px-3 py-2 text-xs text-foreground shadow-sm">
      ...
      {viewModel.groups.length > 0 ? (
        <div className="mt-4 space-y-3">
          {viewModel.groups.map((group) => (
            <div key={group.severity} className="rounded-xl bg-background px-3 py-3 shadow-sm">
              <div className="text-xs font-semibold text-muted-foreground">{group.severityLabel}</div>
              <div className="mt-2 space-y-2">
                {group.issues.map((issue) => (
                  <article key={issue.id} className="rounded-lg bg-muted/60 px-3 py-2">
                    <div className="flex items-start justify-between gap-3">
                    ...
                    <p className="mt-2 text-xs leading-5 text-foreground">{issue.detail}</p>
                    {issue.file ? (
      ...
      {viewModel.reviewContent ? (
        <pre className="mt-4 max-h-48 overflow-auto whitespace-pre-wrap rounded-xl bg-background px-3 py-3 text-xs leading-5 text-foreground shadow-sm">
          {viewModel.reviewContent}
        </pre>
      ) : null}
    </section>
