...
export type ExitPlanPermissionResult = {
  ...
  /** 用户选择的目标权限模式 */
  targetMode?: RV-InsightsPermissionMode
} | {
  behavior: 'deny'
  message: string
}
...
export interface ExitPlanModeCallbacks {
  /** 切换权限模式 */
  onPermissionModeChange: (mode: RV-InsightsPermissionMode) => void
}
...
export class AgentExitPlanService {
   ...
   */
  respondToExitPlanMode(response: ExitPlanModeResponse): { sessionId: string; targetMode: RV-InsightsPermissionMode | null } | null {
