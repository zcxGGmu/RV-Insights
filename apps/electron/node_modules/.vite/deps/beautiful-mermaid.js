    ...
    (function(f) {
      ...
      return (/* @__PURE__ */ (function() {
        ...
        function layout(g, opts) {
          ...
          time("layout", () => {
            let layoutGraph2 = time("  buildLayoutGraph", () => buildLayoutGraph(g));
            time("  runLayout", () => runLayout(layoutGraph2, time, opts));
            time("  updateInputGraph", () => updateInputGraph(g, layoutGraph2));
          });
        }
...
function fromShikiTheme(theme) {
  ...
  const tokenColor = (scope) => {
    var _a, _b, _c;
    return (_c = (_b = (_a = theme.tokenColors) == null ? void 0 : _a.find(
      (t) => Array.isArray(t.scope) ? t.scope.includes(scope) : t.scope === scope
    )) == null ? void 0 : _b.settings) == null ? void 0 : _c.foreground;
  };
  return {
...
function parseStateDiagram(lines) {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    if (transitionMatch) {
      ...
      const edgeLabel = ((_a = transitionMatch[4]) == null ? void 0 : _a.trim()) || void 0;
      if (sourceId === "[*]") {
        startCount++;
...
function parseEdgeLine(line, graph, subgraphStack) {
  ...
  while (remaining.length > 0) {
    ...
    const edgeLabel = ((_a = arrowMatch[3]) == null ? void 0 : _a.trim()) || void 0;
...
function getCanvasSize(canvas) {
  var _a;
  return [canvas.length - 1, (((_a = canvas[0]) == null ? void 0 : _a.length) ?? 1) - 1];
}
...
function mergeJunctions(c1, c2) {
  var _a;
  return ((_a = JUNCTION_MAP[c1]) == null ? void 0 : _a[c2]) ?? c1;
}
...
function parseSequenceDiagram(lines) {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    if (actorMatch) {
      ...
      const label = ((_a = actorMatch[3]) == null ? void 0 : _a.trim()) ?? id;
      if (!actorIds.has(id)) {
        actorIds.add(id);
    ...
    if (blockMatch) {
      ...
      const label = ((_b = blockMatch[2]) == null ? void 0 : _b.trim()) ?? "";
      blockStack.push({
        type: blockType,
        label,
    ...
    if (dividerMatch && blockStack.length > 0) {
      const label = ((_c = dividerMatch[2]) == null ? void 0 : _c.trim()) ?? "";
...
function parseMember(line) {
  ...
  if (methodMatch) {
    ...
    const type2 = (_a = methodMatch[3]) == null ? void 0 : _a.trim();
...
function parseRelationship(line) {
  ...
  const to = match[5];
  const label = ((_a = match[6]) == null ? void 0 : _a.trim()) || void 0;
...
function parseAttribute(line) {
  ...
  const type = match[1];
  const name = match[2];
  const rest = ((_a = match[3]) == null ? void 0 : _a.trim()) ?? "";
  const keys = [];
  let comment;
...
function detectDiagramType(text) {
  var _a;
  const firstLine = ((_a = text.trim().split("\n")[0]) == null ? void 0 : _a.trim().toLowerCase()) ?? "";
...
function preComputeSubgraphLayout(sg, graph, opts) {
  ...
  const edges = subG.edges().map((edgeObj) => {
    ...
    if (rawPoints.length > 0) {
      const srcShape2 = (_a = graph.nodes.get(edgeObj.v)) == null ? void 0 : _a.shape;
      ...
      const tgtShape2 = (_b = graph.nodes.get(edgeObj.w)) == null ? void 0 : _b.shape;
    ...
    const srcShape = (_c = graph.nodes.get(edgeObj.v)) == null ? void 0 : _c.shape;
    const tgtShape = (_d = graph.nodes.get(edgeObj.w)) == null ? void 0 : _d.shape;
...
function extractPositionedGraph(g, graph, padding, preComputed) {
  ...
  const edges = g.edges().map((edgeObj) => {
    ...
    const rawPoints = dagreEdge.points ?? [];
    if (rawPoints.length > 0) {
      const srcShape = (_a = graph.nodes.get(edgeObj.v)) == null ? void 0 : _a.shape;
      if (srcShape === "diamond") {
      ...
      const tgtShape = (_b = graph.nodes.get(edgeObj.w)) == null ? void 0 : _b.shape;
      if (tgtShape === "diamond") {
    ...
    const orthoPoints = snapToOrthogonal(rawPoints, verticalFirst);
    const srcShapeForClip = (_c = graph.nodes.get(edgeObj.v)) == null ? void 0 : _c.shape;
    const tgtShapeForClip = (_d = graph.nodes.get(edgeObj.w)) == null ? void 0 : _d.shape;
...
function extractGroup(g, sg) {
  ...
  return {
    ...
    x: topLeft.x,
    y: topLeft.y,
    width: (dagreNode == null ? void 0 : dagreNode.width) ?? 0,
    height: (dagreNode == null ? void 0 : dagreNode.height) ?? 0,
...
function renderNodeShape(node) {
  ...
  const fill = escapeXml((inlineStyle == null ? void 0 : inlineStyle.fill) ?? "var(--_node-fill)");
  const stroke = escapeXml((inlineStyle == null ? void 0 : inlineStyle.stroke) ?? "var(--_node-stroke)");
  const sw = escapeXml((inlineStyle == null ? void 0 : inlineStyle["stroke-width"]) ?? String(STROKE_WIDTHS.innerBox));
  switch (shape) {
    case "diamond":
...
function renderNodeLabel(node, font) {
  ...
  const textColor = escapeXml(((_a = node.inlineStyle) == null ? void 0 : _a.color) ?? "var(--_text)");
...
function layoutSequenceDiagram(diagram, _options = {}) {
  ...
  const blocks = diagram.blocks.map((block) => {
    ...
    const blockTop = ((startMsg == null ? void 0 : startMsg.y) ?? messageY) - SEQ.blockPadTop;
    const blockBottom = ((endMsg == null ? void 0 : endMsg.y) ?? messageY) + SEQ.blockPadBottom + 12;
    ...
    const dividers = block.dividers.map((d) => {
      ...
      const msgY = (msg == null ? void 0 : msg.y) ?? messageY;
      let offset = 28;
      if (d.label && (msg == null ? void 0 : msg.label)) {
  ...
  const notes = diagram.notes.map((note) => {
    ...
    const noteY = ((refMsg == null ? void 0 : refMsg.y) ?? actorY + SEQ.actorHeight) + 4;
...
//# sourceMappingURL=beautiful-mermaid.js.map
