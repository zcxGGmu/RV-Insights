...
### Pipeline Agent 模型配置（Sprint 5-6 新增）

每个 Pipeline Agent 有独立的 `{AGENT}_MODEL` + `{AGENT}_PROVIDER` 配置对：
...
- 新增 Agent 时遵循此模式：`{AGENT}_MODEL` + `{AGENT}_PROVIDER`
- 每个 node 通过 `_get_{agent}_model_config()` 读取配置并构造 `ModelConfig`
- 默认 fallback 到 `DEFAULT_LLM_MODEL` + `DEFAULT_LLM_PROVIDER`
